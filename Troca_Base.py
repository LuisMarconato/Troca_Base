...
#3.4- CRIAÇÃO DA FUNÇÃO TROCAR
def TROCA():
    #3.4.0-CONDICIONAL QUE ARMAZENA O ENDEREÇO NA VARIAVEL modulo
    ...
    if verifica_servicos_firebird()==True and verifica_servicos_guard()==True:
        ...
        logging.warning('Não parou o serviço no inicio da troca')

    #3.4.2- VERIFICA SE OS SERVIÇOS ESTÃO PARADOS A AÇÃO DE TROCA DE BASE
    if verifica_servicos_firebird()==False and verifica_servicos_guard()==False:
         #3.4.3- RENOMEAR O DATAGES
         os.rename(f'{modulo}/DATAGES.FDB',f'{modulo}/DATAGES_OLD.FDB')
         #3.4.4- BACKUP DO DATAGES DO TECNICO
         shutil.move(f'{modulo}/DATAGES_OLD.FDB', f'{modulo}/Backup Tecnico/DATAGES_OLD.FDB')
         #3.4.5- COPIA O DATAGES DA PASTA BASE CLIENTE PARA A PASTA PASTA GDOOR.
        ...
        logging.warning('Não fez a troca de base')
    #3.4.3- INICIAR O FIREBIRD
    if verifica_servicos_firebird()==False:
        INICIAR_FIREBIRD() 
    
    #3.4.4- VERIFICA SE O FIREBIRD ESTA INICIADO PARA CONSEGUIR CONECTAR
    if verifica_servicos_firebird()==True:
        #3.4.5.1-RODAR O DELETE NA BASE
        ...
        #3.4.6- DESATIVANDO O BOTÃO CONTRÁRIO
        TROCA_button.configure(state='disable')
        REVERTE_button.configure(state='normal')
        
#3.5- CRIAÇÃO DA FUNÇÃO REVERTE
def REVERTER():
    #3.5.1-CONDICIONAL QUE ARMAZENA O ENDEREÇO NA VARIAVEL modulo
    combo = cb_modulos.get()
    if combo == lista_modulos[0]:
        modulo = caminho_pro
    elif combo == lista_modulos[1]:
    ...
    #3.5.3- VERIFICA OS SERIVOÇOS PARA INICIAR A TROCA DA BASE
    if verifica_servicos_firebird()==False and verifica_servicos_guard()==False:    
        #3.3.4-RENOMEAR O DATAGES
    ...
    #3.5.4- INICIAR O GUARD
    if verifica_servicos_firebird()==False:
        INICIAR_FIREBIRD()
    
    #3.5.5-RODAR O DELETE NA BASE
...
def DELETE_VERSAO():
    ...
    #3.6.2- VERIFICAR OS SERVIÇOS PARA CONSEGUIR CONECTAR COM O BANCO
    if verifica_servicos_guard() == True:
        PARAR_GUARD()
        time.sleep(5)
    
    #3.6.3- VERIFICAR OS SERVIÇOS E CONECTAR NO BANCO
    if verifica_servicos_guard() == False:    
